...
const VALID_DAY_IDS = new Set(["mon", "tue", "wed", "thu", "fri", "sat", "sun", "holiday"]);
...
const DEFAULT_CLINIC_HOURS: Availability[] = [
  ...
  { id: "holiday", day: "Holidays",  open_hour: 8, close_hour: 12, is_open: false },
];
  ...
  async function loadProfile() {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Migrate to canonical 8-row format (Mon–Sun + Holidays).
        // Old data may have 7 rows (no holiday row) or non-standard IDs.
        ...
        if (isValidFormat) {
          const loaded = raw as Availability[];
          const hasHoliday = loaded.some((h) => h.id === "holiday");
          setClinicHours(
            hasHoliday
              ? loaded
              : [...loaded, { id: "holiday", day: "Holidays", open_hour: 8, close_hour: 12, is_open: false }]
          );
        } else {
          setClinicHours(DEFAULT_CLINIC_HOURS);
        }
      } else {
